...
TStopwatch timer;
int IndResponse(double kPt=30,const char *ksp="ppb",int ifile=0)
{
...
//  if(strcmp(ksp,"ppb")==0)c = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pPb/HP04/prod16/Hijing_Pythia_pt%.f/HiForest_v77_merged01/pt%.f_HP04_prod16_v77_merged_forest_%d.root",kPt,kPt,ifile),"PythiaHijing",cPPb); 
  ...
  if(kPt==50||kPt==80||kPt==100||kPt==170) c = new HiForest(Form("/mnt/hadoop/cms/store/user/yenjie/HiForest_v27/Dijet%.f_HydjetDrum_v27_mergedV1.root",kPt),"PythiaHydjet",cPbPb);
  else c = new HiForest(Form("/mnt/hadoop/cms/store/user/yenjie/HiForest_v28/Dijet%.f_HydjetDrum_v28_mergedV1.root",kPt),"PythiaHydjet",cPbPb);
  }    
  else {
  ...
  //  c = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pp2013/P02/prod18/Signal_Pythia_pt%.f/HiForest_v79_merged01/pt%.f_pp2013_P02_prod18_v79_merged_forest_0.root",kPt,kPt),"Pythia",cPP);
  //  c = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pp2013/P01/prod22/Signal_Pythia_pt%.f/HiForest_v81_merged01/pt%.f_pp2013_P01_prod22_v81_merged_forest_0.root",kPt,kPt),"Pythia",cPP);
    c = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/ppHiIterativeTrack/P01/prod22/Signal_Pythia_pt%.f/HiForest_v81_merged01/pt%.f_ppHiIterativeTrack_P01_prod22_v81_merged_forest_0.root",kPt,kPt),"Pythia",cPP);
...
//  TFile *fout = new TFile(Form("Output/Response_%s_pT%0.0fGeV_%d.root",ksp,kPt,ifile),"RECREATE");
  TFile *fout = new TFile(Form("Output/pp2760GeVHiIterativeTrackResponse_%s_pT%0.0fGeV_%d.root",ksp,kPt,ifile),"RECREATE");
  ...
  TH1F *hDeltaRSel[knj][nmult];
    ...
    for(int nj=0;nj<knj;nj++){
        ...
        for(int ipt=0;ipt<bins;ipt++){
            hratiocorrrefpt_etaptbin[nj][ipt]= new TH2F(Form("hratiocorrrefpt_etaptbin%d_%d",nj,ipt),Form("Gen matched jet Reco jet CorrPt/ Gen jet p_{T} (corr.) distribution jet vs eta in pt bin %d %s",ipt,cjets[nj]),
