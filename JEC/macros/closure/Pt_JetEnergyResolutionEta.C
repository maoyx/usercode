...
int Pt_JetEnergyResolutionEta(const char *cmode="hijing",int rfit=0)
{
  int statop=0;
  int kRebin=1;
  ...
  //! pp Response
  TH2F *hratiocorrrefpt_pp[knj][nbins];
  TH1F *hratiocorrrefpt1D_pp[knj][nbins][netabins];  
  TH1F *hMean_pp [knj][nbins], *hArM_pp[knj][nbins], *hSigma_pp [knj][nbins], *hRMS_pp[knj][nbins];

    TH2F *hratiorawrefpt_pp[knj][nbins];
    TH1F *hratiorawrefpt1D_pp[knj][nbins][netabins];
    TH1F *hMean_r_pp [knj][nbins], *hArM_r_pp[knj][nbins], *hSigma_r_pp [knj][nbins], *hRMS_r_pp[knj][nbins];

    for(int nj=0;nj<knj;nj++){
    //cout<<"nj : "<<nj<<Form("\t %s",calgo[nj])<<endl;
    
    
        for(int imult=0;imult<nbins;imult++){
            //! pp /////////////////////////////
            hMean_pp [nj][imult] = new TH1F(Form("hMean_pp%d_%d",nj,imult),Form("<reco p_{T}/gen p_{T}> %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hMean_pp[nj][imult],statop);
            
            hArM_pp [nj][imult] = new TH1F(Form("hArM_pp%d_%d",nj,imult),Form("<reco p_{T}/gen p_{T}> %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hArM_pp[nj][imult],statop);
            
            hSigma_pp [nj][imult] = new TH1F(Form("hSigma_pp%d_%d",nj,imult),Form("#sigma(reco p_{T}/gen p_{T}) %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hSigma_pp[nj][imult],statop);
            
            hRMS_pp [nj][imult] = new TH1F(Form("hRMS_pp%d_%d",nj,imult),Form("RMS(reco p_{T}/gen p_{T}) %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hRMS_pp[nj][imult],statop);
            
            hMean_r_pp [nj][imult] = new TH1F(Form("hMean_r_pp%d_%d",nj,imult),Form("<raw p_{T}/gen p_{T}> %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hMean_r_pp[nj][imult],statop);
            
            hArM_r_pp [nj][imult] = new TH1F(Form("hArM_r_pp%d_%d",nj,imult),Form("<raw p_{T}/gen p_{T}> %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hArM_r_pp[nj][imult],statop);
            
            hSigma_r_pp [nj][imult] = new TH1F(Form("hSigma_r_pp%d_%d",nj,imult),Form("#sigma(raw p_{T}/gen p_{T}) %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hSigma_r_pp[nj][imult],statop);
            
            hRMS_r_pp [nj][imult] = new TH1F(Form("hRMS_r_pp%d_%d",nj,imult),Form("RMS(raw p_{T}/gen p_{T}) %s pp %d",calgo[nj],imult),netabins,etabins);
            MakeHist(hRMS_r_pp[nj][imult],statop);
            
            hratiocorrrefpt_pp[nj][imult]  = (TH2F*)fin_pPb->Get(Form("hratiocorrrefpt_etaptbin%d_%d",nj,imult));
            hratiocorrrefpt_pp[nj][imult]->SetName(Form("hratiocorrrefpt_pp%d_%d",nj,imult));
            //hratiocorrrefpt_pp[nj][imult]->Rebin2D(kRebin,2);
            
            hratiorawrefpt_pp[nj][imult]  = (TH2F*)fin_pPb->Get(Form("hratiorawrefpt_etaptbin%d_%d",nj,imult));
            hratiorawrefpt_pp[nj][imult]->SetName(Form("hratiorawrefpt_pp%d_%d",nj,imult));
            
            for(int ip=0;ip<netabins;ip++){
                int lbin = (int)hratiocorrrefpt_pp[nj][imult]->GetXaxis()->FindBin(etabins[ip]);
                int hbin = (int)hratiocorrrefpt_pp[nj][imult]->GetXaxis()->FindBin(etabins[ip+1])-1;
  ...
  for(int nj=0;nj<knj;nj++){
    for(int ic=0;ic<nbins;ic++){
      ...
      MakeHistRMS (hSigma_pp[nj][ic],0.43,0.0001);
        ...
        MakeHistMean(hArM_r_pp  [nj][ic],1.388,0.64);
        MakeHistRMS (hRMS_r_pp  [nj][ic],0.53,0.0001);
        MakeHistMean(hMean_r_pp [nj][ic],1.388,0.64);
        MakeHistRMS (hSigma_r_pp[nj][ic],0.53,0.0001);
    }
...
//    TPaveText *pt = new TPaveText(0.4273642,0.017537,0.6165358,0.1545994,"brNDC");
//    pt->SetBorderSize(0);
//    pt->SetFillColor(10);
//    pt->SetTextFont(42);
//
...
//    pt1->SetTextFont(42);
    ...
    for(int ic=0;ic<nbins;ic++){
        c1[ic] = new TCanvas(Form("c1_%d",ic),Form("%.f-%.f RAW JES JER",ptbins[ic],ptbins[ic+1]),3,230,1465,605);
        ...
        for(int nj=1;nj<4;nj++){
            ...
            if(ipad==1){
                ...
                pt->SetFillColor(10);
                pt->SetTextFont(42);
                TText *text = pt->AddText(Form("%.f < p_{T}^{jet} < %.f (GeV/c)",ptbins[ic],ptbins[ic+1]));
        ...
        if(SavePlot){
            c1[ic]->Print(Form("%s/%s%sJetPt%.f_%.fPFJetRAWJESJER.pdf", plotsdir.Data(), COM.Data(), gen.Data(), ptbins[ic], ptbins[ic+1]));
  ...
  for(int ic=0;ic<nbins;ic++){
    c3[ic] = new TCanvas(Form("c3_%d",ic),Form("%.f-%.f JES JER",ptbins[ic],ptbins[ic+1]),3,230,1465,605);
...
//	  text4->SetTextSize(0.09);
	...
	text2->SetTextSize(0.09);
	text2->SetTextColor(2);
//	TText *text3 = pt1->AddText(Form("%.f < p_{T}^{jet} < %.f (GeV/c)",ptbins[ic],ptbins[ic+1]));
//	text3->SetTextSize(0.09);
	 pt1->Draw();
...
//      line->SetLineStyle(2);
      ...
      if(SavePlot){
          c3[ic]->Print(Form("%s/%s%sJetPt%.f_%.fPFJetJESJER.pdf", plotsdir.Data(), COM.Data(), gen.Data(), ptbins[ic], ptbins[ic+1]));
  ...
  TCanvas *c4[nbins];
  for(int ic=0;ic<nbins;ic++){
    c4[ic] = new TCanvas(Form("c4_%d",ic),Form("%.f-%.f Calo JES JER",ptbins[ic],ptbins[ic+1]),3,230,1465,605);
...
//	text4->SetTextSize(0.09);
	pt3->Draw();
      ...
      if(ipad==1){
	TPaveText *pt = new TPaveText(0.4273642,0.017537,0.6165358,0.1045994,"brNDC");
	pt->SetBorderSize(0);
	pt->SetFillColor(10);
	pt->SetTextFont(42);
    TText *text = pt->AddText(Form("%.f < p_{T}^{jet} < %.f (GeV/c)",ptbins[ic],ptbins[ic+1]));
...
//      line->SetLineStyle(2);
      ...
      if(SavePlot){
          c4[ic]->Print(Form("%s/%s%sJetPt%.f_%.fCaloJetJESJER.pdf", plotsdir.Data(),COM.Data(), gen.Data(), ptbins[ic], ptbins[ic+1]));
    ...
    */
    ...
    if(SaveFile){
        TFile * outf = new TFile(Form("%sJESJEREtaClosure.root", gen.Data()), "CREATE");
        for(int ic=0;ic<nbins;ic++){
            for(int nj=1;nj<4;nj++){
                hRMS_pp[nj][ic]->Write(Form("%sSigmaJetPt%.f_%.f", algn[nj], ptbins[ic], ptbins[ic+1]));
                hRMS_pp[nj+4][ic]->Write(Form("%sSigmaJetPt%.f_%.f", algn[nj+4], ptbins[ic], ptbins[ic+1]));
                hRMS_pp[nj+8][ic]->Write(Form("%sSigmaJetPt%.f_%.f", algn[nj+8], ptbins[ic], ptbins[ic+1]));
                hRMS_pp[nj+12][ic]->Write(Form("%sSigmaJetPt%.f_%.f", algn[nj+12], ptbins[ic], ptbins[ic+1]));
                hArM_pp[nj][ic]->Write(Form("%sMeanJetPt%.f_%.f", algn[nj], ptbins[ic], ptbins[ic+1]));
                hArM_pp[nj+4][ic]->Write(Form("%sMeanJetPt%.f_%.f", algn[nj+4], ptbins[ic], ptbins[ic+1]));
                hArM_pp[nj+8][ic]->Write(Form("%sMeanJetPt%.f_%.f", algn[nj+8], ptbins[ic], ptbins[ic+1]));
                hArM_pp[nj+12][ic]->Write(Form("%sMeanJetPt%.f_%.f", algn[nj+12], ptbins[ic], ptbins[ic+1]));
...
void MakeHist(TH1 *histo,int istat)
{
  ...
  histo->GetXaxis()->SetTitle(" GenJet #eta ");
...
void FillMeanSigma(int ip,TH1 *h1F,TH1 *hArM,TH1 *hRMS,TH1 *hMean,TH1 *hSigma)
{
 ...
 //   if(h1F->GetEntries()<maxEntry) break ;  
