...
public:
    ...
    TH2F * trackphi[nbin];
    TH2F * tracketa[nbin];
    TH1F * trackpteff[nbin];
    TH1F * bkgtrackpteff[nbin];
    TH1F * trackdreff[nbin];
    TH1F * bkgtrackdreff[nbin];
   ...
   //   else   jetpt[ibin] = new TH1F(Form("jetpt_%d-%d%%",centr[ibin],centr[ibin+1]), Form("jetpt_%d-%d%%",centr[ibin],centr[ibin+1]), nJetPtBin, jetPtBin);
     ...
     //   trackpt[ibin] = new TH2F(Form("trackpt_%d-%d%%",centr[ibin],centr[ibin+1]), Form("trackpt_%d-%d%%",centr[ibin],centr[ibin+1]), 500, 0., 500, 200, 0., 200); 
      ...
      //  jetbkgfrag[ibin]->GetYaxis()->SetTitle("#xi = ln(p_{T}^{jet}/p_{T}^{h})");
        ...
        trackpteff[ibin] = new TH1F(Form("jettrackpteff_%d-%d%%",centr[ibin],centr[ibin+1]), Form("jettrackpteff_%d-%d%%",centr[ibin],centr[ibin+1]), nPtBin, TrkBin);
        trackpteff[ibin]->GetXaxis()->SetTitle("p_{T}^{trk} (GeV/c)");
        trackpteff[ibin]->Sumw2();
        bkgtrackpteff[ibin] = new TH1F(Form("bkgtrackpteff_%d-%d%%",centr[ibin],centr[ibin+1]), Form("bkgtrackpteff_%d-%d%%",centr[ibin],centr[ibin+1]), nPtBin, TrkBin);
        bkgtrackpteff[ibin]->GetXaxis()->SetTitle("p_{T}^{trk} (GeV/c)");
        bkgtrackpteff[ibin]->Sumw2();
        trackdreff[ibin] = new TH1F(Form("jettrackdreff_%d-%d%%",centr[ibin],centr[ibin+1]), Form("jettrackdreff_%d-%d%%",centr[ibin],centr[ibin+1]), (Int_t)(conesize*100/(deltacone*100)), 0., conesize);
        trackdreff[ibin]->GetXaxis()->SetTitle("radius r");
        trackdreff[ibin]->Sumw2();
        bkgtrackdreff[ibin] = new TH1F(Form("bkgtrackdreff_%d-%d%%",centr[ibin],centr[ibin+1]), Form("bkgtrackdreff_%d-%d%%",centr[ibin],centr[ibin+1]), (Int_t)(conesize*100/(deltacone*100)), 0., conesize);
...
void hist_class::Write()
{
    ...
    if(doTrackCorrections){
     if(IsMC==kTRUE && coll=="PP")   
          out_name=Form("%s%s_Ak3PF%sLeadingJetPt%.f_%sIterTrkCorrtest%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%d_trkbin%dpthat%d_%s",dataType.Data(),coll.Data(),level.Data(),leadingjetcut,corrMet.Data(), trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin,pthat, intputFile.Data());
       else 
         out_name=Form("%s%s_AkPu3PF%sLeadingJetPt%.f_%sIterTrkCorrtest%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%d_%s",dataType.Data(),coll.Data(),level.Data(),leadingjetcut,corrMet.Data(), trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin, intputFile.Data());
      }
       else {
    if(IsMC==kTRUE && coll=="PP")
            out_name=Form("%s%s_Ak3PF%sLeadingJetPt%.f_Trk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%dpthat%d_%s",dataType.Data(),coll.Data(),level.Data(),leadingjetcut,trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin,pthat, intputFile.Data());
    else        
            out_name=Form("%s%s_Ak3PF%sLeadingJetPt%.f_Trk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%d_%s",dataType.Data(),coll.Data(),level.Data(),leadingjetcut,trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin, intputFile.Data());       
    }
     ...
     //   }
        ...
        for(int j4i = 0; j4i < my_ct->nref ; j4i++) {
          ...
          //  if(flavor!=21) continue ;  //only gluon jets selected
           ...
           }  // ! end of jet loop for leading search
           if(LeadingIndex>-1){
              jet_pt = my_ct->jtpt[LeadingIndex];
              jet_phi =my_ct->jtphi[LeadingIndex];
            if(jet_phi<=-TMath::Pi())jet_phi+=TMath::TwoPi();
            jet_eta = my_ct->jteta[LeadingIndex];
...
//                genjet_eta =my_ct->refeta[genj4i];  
                    ...
                    genjet_pt =my_ct->refpt[LeadingIndex];  
                    genjet_phi =my_ct->refphi[LeadingIndex]; 
                 if(genjet_phi<=-TMath::Pi())genjet_phi+=TMath::TwoPi();            
                    genjet_eta =my_ct->refeta[LeadingIndex];  
                ...
                for(int nic=0;nic<4;nic++){
                    ...
                    if(my_hists->IsMC==kTRUE){
                     ...
                     //   fsmpt = GetSmearedPtData_woAfBurn(2,nic,my_ct->jtpt[j4i],0, "no");
                        fsmpt = GetSmearedPtData(2,nic,my_ct->jtpt[LeadingIndex],0, "no");
                    //  fsmpt = GetSmearedPtData_NoMeanShift(2,nic,jet_pt,0, "no");
                        fjetsmf = GetSmFactor(2, nic, my_ct->jtpt[LeadingIndex]) ;
                        jetsms = GetMeanShift(2,nic, my_ct->jtpt[LeadingIndex]) ;
                 ...
                 /*   else if(nbin==4){
                    ...
                    if(my_hists->IsMC==kTRUE){
                       ...
                       my_hists->jetms[ibin]->Fill(genjet_pt, jetsms, weight);
                        ...
                        my_hists->jetsmf[ibin]->Fill(my_ct->jtpt[LeadingIndex], fjetsmf, weight);
                        my_hists->jetms[ibin]->Fill(my_ct->jtpt[LeadingIndex], jetsms, weight);
                    }
                    ...
                    if(TMath::Abs(my_ct->jteta[LeadingIndex])>etalimit) { //continue ;
                        ...
                        for(int itr = 0 ; itr < my_tr->nTrk ; itr++){
                           ...
                           //     if(TMath::Abs(delr-dr-TMath::TwoPi())<=conesize)cout <<"two way dr calculation are:" <<"Vector3 =" <<dr <<" Yaxian ="<<delr<<endl ;
                                ...
                                if(dr<=conesize){  //for leading jet shape study
                                    if(TMath::Abs(jet_eta)>=etalimit) my_hists->trackpteff[ibin]->Fill(tr_pt,tr_pt*trkweight); 
                                    ...
                                    if(bkg_dr<=conesize){
                                      if(TMath::Abs(jet_eta)>=etalimit) my_hists->bkgtrackpteff[ibin]->Fill(tr_pt,tr_pt*trkweight);   
                ...
                if(TMath::Abs(my_ct->jteta[LeadingIndex])>etalimit) { //continue ;
                  ...
                  //  if(TMath::Abs(delr-dr-TMath::TwoPi())<=conesize)cout <<"two way dr calculation are:" <<"Vector3 =" <<dr <<" Yaxian ="<<delr<<endl ;                            
                     ...
                     //    if(curr_bin==nbin-1 && evi <10000) cout << ", evi: " << evi << " of " << c->GetEntries() << "  icentrality = " <<curr_bin << "   jet index : " << j4i << "   jetpt : " <<  jet_pt << " jet eta : "  << jet_eta <<  "   jetphi :  " << jet_phi <<    "   trkId : " << itr <<  "   trkpt : " << tr_pt<<  "   trk weight : " << c->getTrackCorrection(itr) << "  trkEta: " << tr_eta << "   trkPhi : " <<  tr_phi << "    deltaR : " << dr  << endl;
                                trackcount++;
                                if(TMath::Abs(jet_eta)>=etalimit) my_hists->trackpteff[curr_bin]->Fill(tr_pt,tr_pt*trkweight);   
                                ...
                                if(bkg_dr<=conesize){
                                    if(TMath::Abs(jet_eta)>=etalimit) my_hists->bkgtrackpteff[curr_bin]->Fill(tr_pt,tr_pt*trkweight);   
